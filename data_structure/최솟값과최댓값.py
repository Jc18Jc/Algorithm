...
def build1(node, start, end):
    if start == end:
        # 리프 노드라면 원소를 저장한다.
        tree1[node] = numArray[start]
        return
    mid = (start + end) // 2
    # 왼쪽 자식으로 재귀
    ...
    # 내부 노드라면 자식 노드 중 큰 값을 저장한다.
    tree1[node] = tree1[node*2] if tree1[node*2] > tree1[node*2+1] else tree1[node*2+1]
    return
...
def query1(node, start, end, left, right):
    ...
    mid = (start + end)//2
    left_child = query1(node*2, start, mid, left, right)
    right_child = query1(node*2+1, mid+1, end, left, right)
...
for i in range(m):
    ...
    print(query2(1, 0, n-1, a-1, b-1), query1(1, 0, n-1, a-1, b-1))
